...
const PUBLIC_BASE_URL = process.env.PUBLIC_BASE_URL || 'https://localhost:8091';
...
const PIN_PASSPHRASE = 'LongSecurePassphraseForArabBankPINs';
    ...
    try {
      ...
      if (pinResult.recordset.length > 0) {
        ...
        const response = await fetch(
          'https://api-int.gcptest.arabbank.plc/digital-banking/ms/security/key-generator/v2/rsa/keys',
          {
            method: 'POST',
            headers: {
              ...
              client_id: 'DAAEC9nldWcy4zR1zPAtsk2pKBiFHfgzrhAhjjRNuBQGm6rA',
            },
  ...
  app.put('/api/customer/pin/change', basicAuth, async (req, res) => {
    ...
    try {
      ...
      const rsaRes = await fetch(
        `https://api-int.gcptest.arabbank.plc/digital-banking/ms/security/key-generator/v2/rsa/keys/${kid}?country=JO&scope=helios`,
        {
          method: 'GET',
          headers: {
            accept: 'application/json',
